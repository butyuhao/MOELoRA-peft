...
TOKENIZER_CONFIGS = {
    ...
    "P01son/Linly-Chinese-LLaMA-13b-hf": TokenizerConfig(special_tokens=SpecialTokens("</s>", "</s>", sep_token="<s>")),
    "BelleGroup/BELLE-7B-2M": TokenizerConfig(special_tokens=SpecialTokens("</s>", "</s>", sep_token="<s>")),
...
def get_tokenizer(conf) -> transformers.AutoTokenizer:
    ...
    if 'baichuan' in conf.model_name.lower():
        from transformers import AutoTokenizer
        tokenizer = AutoTokenizer.from_pretrained(conf.model_name, trust_remote_code=True)
        tokenizer_config = match_tokenizer_name("llama")
    else:
        from transformers import LlamaTokenizer
        tokenizer = LlamaTokenizer.from_pretrained(tokenizer_name, cache_dir=conf.cache_dir)
        tokenizer_config = match_tokenizer_name("llama")
    ...
    # if "llama" in conf.model_name.lower():
...
def get_dataset(
    ...
    for data_config in conf["datasets"]:
        ...
        # print("*"*66,get_dataset_name_and_kwargs_from_data_config(data_config)[1])
        train, val = get_one_dataset(conf, dataset_name, mode=mode, **kwargs)
        train_datasets.append(train)

        if val is not None:
            evals[dataset_name] = Subset(val, list(range(min(len(val), conf["eval_size"])))) if "eval_size" in conf else val
