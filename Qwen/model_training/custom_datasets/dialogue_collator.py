...
- Socrates: Disable.'''
    ...
    def preprocess_internlm(
        ...
        _user = tokenizer('user').input_ids[1:] + nl_tokens
        _assistant = tokenizer('assistant').input_ids[1:] + nl_tokens
        ...
        for i, source in enumerate(sources):
            ...
            for j, sentence in enumerate(source):
                role = roles[sentence["from"]]
                _input_id = tokenizer(role).input_ids[1:] + nl_tokens + \
                    tokenizer(sentence["value"]).input_ids[1:] + [im_end] + nl_tokens
                input_id += _input_id
                if role == '<|im_start|>user':
                    _target = [im_start] + [IGNORE_TOKEN_ID] * (len(_input_id)-3) + [im_end] + nl_tokens
                elif role == '<|im_start|>assistant':
                    _target = [im_start] + [IGNORE_TOKEN_ID] * len(tokenizer(role).input_ids[1:]) + \
                        _input_id[len(tokenizer(role).input_ids[1:])+1:-2] + [im_end] + nl_tokens
                else:
                    raise NotImplementedError
                target += _target
    ...
    def __call__(self, features):
        ...
        for messages in features:
            if isinstance(messages,tuple) and len(messages)==3 and messages[-1]=="<|CustomData|>":
                ...
                batch_system_prompt.append(messages[1])
            else:
                dialogue = messages
                dialogue = [{"from": "user", "value": dialogue[i]} if i%2==0 else {"from": "assistant", "value": dialogue[i]} for i, t in enumerate(dialogue)]
                batch_dialogue.append(dialogue)
        ...
        batch_data = self.preprocess(batch_dialogue, batch_system_prompt)
